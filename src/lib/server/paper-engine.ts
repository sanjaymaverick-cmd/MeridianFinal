...
import {
  decide, manage, sessionClock, PAPER_BUDGET, MAX_POS_PAPER, COOLDOWN_SEC,
  MAX_HEAT, SIZE_FLOOR, SIZE_CEIL, DESK_PROFILE, type Position,
...
type Fill = { id: string; ts: number; symbol: string; side: "BUY" | "SELL"; qty: number; price: number; reason: string };
type ScanRow = { symbol: string; action: string; reason: string; metaProb: number; px: number };
...
function cryptoStub(sym: string, last: number): UniverseName {
  ...
  return {
    symbol: sym, name: `${sym}/USDT`, last, sma20: last, sma50: last,
    high20: last * 1.05, low20: last * 0.95, rsi: 50, atrPct: 0.04,
    quality: 5.4, ownership: 5, sentiment: 6, themes: ["crypto"],
    thesis: "Binance USDT spot. Paper, live last only.", sector: "Crypto",
    assetClass: "crypto", quote: "USD", venue: "Binance",
  };
}

const CASH_WATCH = ["HDFCBANK","ICICIBANK","RELIANCE","TCS","INFY","LT","POLYCAB","GOLD","CRUDE","USDINR","NIFTYFUT"];
...
function activeWatch(cryptoSymbols: string[]) {
  ...
  if (DESK_PROFILE === "pnl") {
    const liveMajors = cryptoSymbols.filter((s) => PNL_CRYPTO.includes(s) || s.endsWith("PERP"));
    const crypto = liveMajors.length ? liveMajors : PNL_CRYPTO.slice(0, 8);
...
function emptyEngine(): Engine {
  ...
  return {
    mode: "auto", killed: false, positions: [], fills: [], ticks,
    anchors: { ...ticks }, dailyPnl: 0, scan: [], samples: 0, lastTick: 0,
    ticksRun: 0, cooldownUntil: {}, live: {},
  };
}
...
async function persistFill(f: Fill, metaProb: number, pnl: number | null) {
  try {
    ...
    await sql`insert into paper_fills (id, ts, symbol, side, qty, price, reason, meta_prob, pnl)
...
async function persistSample(row: Record<string, unknown>) {
  try {
    ...
    const sql = await getSql();
    await sql`insert into paper_samples (
      ...
      ${row.id as string}, ${new Date(row.tsOpen as number).toISOString()}, ${new Date(row.tsClose as number).toISOString()},
      ${row.symbol as string}, ${row.side as string}, ${row.qty as number}, ${row.entry as number}, ${row.exit as number},
      ${row.pnl as number}, ${row.holdSec as number}, ${row.fwdRet as number}, ${row.reasonOpen as string},
      ${row.reasonClose as string}, ${row.metaProb as number}, ${row.confidence as number}, ${row.confluence as number},
      ${row.pSuccess as number}, ${row.atrPct as number}, ${row.score as number}, ${JSON.stringify(row)}
    )`;
  } catch (err) {
...
async function tick() {
  ...
  try {
    ...
    Object.assign(live, await refreshBinanceAnchors());
  ...
  for (const pos of positions) {
    ...
    if (!(livePx > 0) && !(night && !crypto)) { still.push(pos); continue; }
    const px = livePx > 0 ? livePx : (eng.ticks[pos.symbol] ?? pos.entryPrice);
    const intent = night && !crypto
    ...
    if (intent.action === "SELL") {
      ...
      const fill: Fill = {
        ...
        ts: now, symbol: pos.symbol, side: closeSide, qty: pos.qty, price: px,
      ...
      await persistSample({
        id: fill.id, tsOpen: pos.entryTs, tsClose: now, symbol: pos.symbol, side: pos.side,
        qty: pos.qty, entry: pos.entryPrice, exit: px, pnl, holdSec, fwdRet,
        reasonOpen: pos.reasonOpen, reasonClose: intent.reason, metaProb: pos.metaProb,
        confidence: pos.confidence, confluence: pos.confluence, pSuccess: pos.pSuccess,
        atrPct: pos.atrPct, score: pos.score, label: fwdRet > 0 ? 1 : 0,
      });
    } else {
      still.push({
        ...pos,
  ...
  if ((eng.mode === "auto" || eng.mode === "paper") && !eng.killed) {
    const ranked = activeWatch(Object.keys(eng.live)).map((sym) => {
      ...
      const intent = decide(
        {
          symbol: sym, confidence: Math.min(0.88, score / 10 + 0.06), confluence: 72 + (score - 6),
          pSuccess, atrPct: u.atrPct || 0.016, minutesToEod: clock.minutesToEod,
          minutesSinceMidnight: 550, beliefPosterior: 0.25, portfolioHeat: heat,
        },
        { dailyPnl: eng.dailyPnl, killed: eng.killed, live: false, nOpen: positions.length, cooldownUntil: eng.cooldownUntil },
        now,
      );
      return { sym, px: liveLast, score, pSuccess, intent, u };
    }).filter(Boolean) as Array<{
      sym: string; px: number; score: number; pSuccess: number;
      intent: ReturnType<typeof decide>; u: (typeof UNIVERSE)[number];
    }>;

    for (const row of ranked) {
      scan.push({ symbol: row.sym, action: row.intent.action, reason: row.intent.reason, metaProb: row.intent.metaProb, px: row.px });
    }
    ...
    for (const row of openable) {
      ...
      if (qty <= 0) continue;
      const pos: Position = {
        symbol: row.sym, side, entryPrice: px, entryTs: now,
        stopPct: long ? row.intent.stopPct : 0.012, sizePct, metaProb: row.intent.metaProb,
        highSinceEntry: px, lowSinceEntry: px, qty,
        reasonOpen: long ? row.intent.reason : "fade_short",
        confidence: Math.min(0.88, row.score / 10 + 0.06), confluence: 72,
        pSuccess: row.pSuccess, atrPct: row.u.atrPct || 0.016, score: row.score,
      };
      positions.push(pos);
      runningHeat += sizePct;
      const fill: Fill = {
        id: `${now}-${row.sym}-${side}-${qty}`, ts: now, symbol: row.sym,
        side: side === "long" ? "BUY" : "SELL", qty, price: px, reason: `${pos.reasonOpen}:live`,
      };
      eng.fills = [fill, ...eng.fills].slice(0, 400);
  ...
  try {
    ...
    await writeFile(HEARTBEAT, JSON.stringify({
      ts: eng.lastTick, mode: eng.mode, killed: eng.killed, open: eng.positions.length,
      samples: eng.samples, ticksRun: eng.ticksRun, liveNames: Object.keys(eng.live).length,
      source: "binance-live", profile: DESK_PROFILE,
    }, null, 2));
  } catch { /* ignore */ }
}
...
export function startPaperEngine() {
  ...
  if (g.__meridianPaper?.rev === ENGINE_REV) return;
  if (g.__meridianPaper) clearInterval(g.__meridianPaper.timer);
  const eng = g.__meridianPaper?.eng ?? emptyEngine();
  if (!g.__meridianPaper?.eng) { eng.mode = "auto"; eng.killed = false; }
  const timer = setInterval(() => { tick().catch((err) => console.error("[paper] tick", err)); }, 2500);
...
export function snapshotBook(): PaperBook {
  const e = getEngine();
  return {
    mode: e.mode, killed: e.killed, positions: e.positions, fills: e.fills.slice(0, 80),
    ticks: e.ticks, dailyPnl: e.dailyPnl, scan: e.scan, samples: e.samples,
    lastTick: e.lastTick, ticksRun: e.ticksRun,
  };
}
...
export async function listSamples(limit = 500) {
  const sql = await getSql();
  return sql`select id, ts_open, ts_close, symbol, side, qty, entry, exit, pnl, hold_sec, fwd_ret,
    reason_open, reason_close, meta_prob, score from paper_samples order by ts_close desc limit ${limit}`;
}
