...
No Node.js install. Double-click **`MeridianFinal-Setup-1.0.3.exe`** (build it with `npm run dist:win` from this repo). Do not install into Program Files — Vite needs a writable folder. Per-user install is the default. If an older copy is already under Program Files, uninstall it first; 1.0.3 will also mirror the desk into `%LOCALAPPDATA%\Meridian Final\runtime`.
